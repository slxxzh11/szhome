...
public class FileSetDownloadingAndRelativePathsStoringBackup extends Thread {
	...
	public FileSetDownloadingAndRelativePathsStoringBackup(HashSet urls,
...
//				+ urls.size());
		while (remainderIterator.hasNext()) {
			...
			if(redownloadCount.get(urlNotStoreInMap)==null){
				...
				if(count.length()>3){
					System.out.println("there are three thread downloading :" + urlNotStoreInMap);
					continue;					
				}				
